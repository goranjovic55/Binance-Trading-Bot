...
def analyze(pairs):
    ...
    for pair in pairs:
        ...
        if (
            pair in last_RSI
            and (RSI - last_RSI[pair] >= 2.5)
            and (RSI >= 49 and RSI <= 67)
            and (STOCH_RSI_K >= 25 and STOCH_RSI_K <= 58)
            and """(EMA10 > EMA20 and EMA20 > EMA100)"""
            and (STOCH_K - STOCH_D >= 4.5)
        ):
            ...
            if oscCheck >= OSC_THRESHOLD and maCheck >= MA_THRESHOLD:
                ...
                if FULL_LOG:
                    print(
                        f"\033[92mCustsignalmod: Signal detected on {pair} at {oscCheck}/{len(OSC_INDICATORS)} oscillators and {maCheck}/{len(MA_INDICATORS)} moving averages."
                    )
