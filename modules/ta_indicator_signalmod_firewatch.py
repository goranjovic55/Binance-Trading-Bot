...
def analyze(pairs):
    ...
    for pair in pairs:
        ...
        if FULL_LOG:
            print(
                f"buysellcustsignal:{pair} First {first_tacheck} Second {second_tacheck} Third {third_tacheck}"
            )
            print(
                f"buysellcustsignal:{pair} First {first_recommendation} Second {second_recommendation} Third {third_recommendation}"
            )
        # else:
        # print(".", end = '')
        ...
        if (
            (first_recommendation == "BUY" or first_recommendation == "STRONG_BUY")
            and (
                second_recommendation == "BUY" or second_recommendation == "STRONG_BUY"
            )
            and (third_recommendation == "BUY" or third_recommendation == "STRONG_BUY")
        ):
        ...
        if (
            (first_recommendation == "SELL" or first_recommendation == "STRONG_SELL")
            and (
                second_recommendation == "SELL"
                or second_recommendation == "STRONG_SELL"
            )
            and (
    ...
    while True:
        if FULL_LOG:
            ...
            if len(signal_coins) == 0:
                print(
                    f"buysellcustsignal: No coins above {TA_BUY_THRESHOLD} threshold on three timeframes. Waiting {TIME_TO_WAIT} minutes for next analysis"
                )
            else:
                print(
                    f"buysellcustsignal: {len(signal_coins)} coins above {TA_BUY_THRESHOLD} treshold on three timeframes. Waiting {TIME_TO_WAIT} minutes for next analysis"
                )
