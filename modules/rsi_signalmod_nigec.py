...
)  # Main Timeframe for analysis on Oscillators and Moving Averages (15 mins)
INTERVAL2 = (
    Interval.INTERVAL_5_MINUTES
)  # Secondary Timeframe for analysis on BUY signals for next lowest timescale | Check Entry Point (5)
...
MA_SUMMARY = (
    13  # Buy indicators out of 26 to use in Moving Averages INTERVAL analysis (13)
)
MA_SUMMARY2 = (
    13  # Buy indicators out of 26 to use in Moving Averages INTERVAL2 analysis (13)
)
...
SIGNALS_SELL = (
    7  # Max number of buy signals on both INTERVALs to add coin to sell list (7)
)
...
def analyze(pairs):
    ...
    for pair in pairs:
        ...
        if (RSI >= RSI_MIN and RSI <= RSI_MAX) and (RSI_DIFF >= RSI_BUY):
            if (
                (STOCH_DIFF >= STOCH_BUY)
                and (STOCH_K >= STOCH_MIN and STOCH_K <= STOCH_MAX)
                and (STOCH_D >= STOCH_MIN and STOCH_D <= STOCH_MAX)
            ):
                if (
                    (BUY_SIGS >= MA_SUMMARY)
                    and (BUY_SIGS2 >= MA_SUMMARY2)
                    and (STOCH_K > STOCH_K1)
                ):
        ...
        if SELL_COINS:
            if (
                (BUY_SIGS < SIGNALS_SELL)
                and (BUY_SIGS2 < SIGNALS_SELL)
                and (STOCH_DIFF < STOCH_SELL)
                and (RSI_DIFF < RSI_SELL)
                and (STOCH_K < STOCH_K1)
            ):
                # signal_coins[pair] = pair
