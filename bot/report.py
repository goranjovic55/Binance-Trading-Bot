...
def report(type: str, reportline: str) -> None:
    ...
    INVESTMENT_VALUE_GAIN_TRIM = "%g" % round(
        session_struct["investment_value_gain"], 2
    )
    CURRENT_EXPOSURE_TRIM = "%g" % session_struct["CURRENT_EXPOSURE"]
    INVESTMENT_TOTAL_TRIM = "%g" % INVESTMENT_TOTAL
    CLOSED_TRADES_PERCENT_TRIM = "%g" % round(
        session_struct["closed_trades_percent"], 2
    )
    ...
    SETTINGS_STRING = (
        "Time: "
        + str(round(settings_struct["TIME_DIFFERENCE"], 2))
        + " | Interval: "
        + str(round(settings_struct["RECHECK_INTERVAL"], 2))
        + " | Price change min/max: "
        + str(round(settings_struct["CHANGE_IN_PRICE_MIN"], 2))
        + "/"
        + str(round(settings_struct["CHANGE_IN_PRICE_MAX"], 2))
        + "% | Stop loss: "
        + str(round(settings_struct["STOP_LOSS"], 2))
        + " | Take profit: "
        + str(round(settings_struct["TAKE_PROFIT"], 2))
        + " | Trailing stop loss: "
        + str(round(settings_struct["TRAILING_STOP_LOSS"], 2))
    ...
    report_string = (
        "Trade slots: "
        + str(session_struct["trade_slots"])
        + "/"
        + str(TRADE_SLOTS)
        + " ("
        + str(CURRENT_EXPOSURE_TRIM)
        + "/"
        + str(INVESTMENT_TOTAL_TRIM)
        + " "
        + PAIR_WITH
        + ") | Session: "
        + str(SESSION_PROFIT_TRIM)
        + " "
        + PAIR_WITH
        + " ("
        + str(CLOSED_TRADES_PERCENT_TRIM)
        + "%) | Win/Loss: "
        + str(WON)
        + "/"
        + str(LOST)
        + " ("
        + str(WIN_LOSS_PERCENT)
        + "%) | Gains: "
        + str(round(session_struct["INVESTMENT_GAIN"], 4))
        + "%"
        + " | Balance: "
        + str(NEW_BALANCE_TRIM)
        + " | Value: "
        + str(INVESTMENT_VALUE_TRIM)
        + " USD | Value gain: "
        + str(INVESTMENT_VALUE_GAIN_TRIM)
        + " | Uptime: "
