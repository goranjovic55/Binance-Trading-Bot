    ...
    if type == 'detailed':
        print(
            ...
            , f"Change in Price:        {txcolors.DEFAULT}"
              f"{session_struct['CHANGE_IN_PRICE_MIN']:.2f} - {session_struct['CHANGE_IN_PRICE_MAX']:.2f}\n"
            , f"Stop Loss:              {txcolors.DEFAULT}"
              f"{session_struct['STOP_LOSS']:.2f}\n"
            , f"Take Profit:            {txcolors.DEFAULT}"
              f"{session_struct['TAKE_PROFIT']:.2f}\n"
            , f"Trailing Stop Loss:     {txcolors.DEFAULT}"
              f"{session_struct['TRAILING_STOP_LOSS']:.2f}\n"
            , f"Trailing Take Profit:   {txcolors.DEFAULT}"
              f"{session_struct['TRAILING_TAKE_PROFIT']:.2f}\n"
