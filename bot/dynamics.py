...
# Load creds modules
from helpers.handle_creds import load_correct_creds, test_api_key, load_telegram_creds
...
def dynamic_settings(type, TIME_DIFFERENCE, RECHECK_INTERVAL):
    ...
    if DYNAMIC_SETTINGS:
        ...
        settings_struct["STOP_LOSS"] = (
            settings_struct["STOP_LOSS"] + DYNAMIC_STOP_LOSS / 2
        )
        ...
        if (
            session_struct["last_trade_won"] is True
            and session_struct["dynamics_state"] == "up"
        ):
            type = "performance_adjust_up"

        if (
            session_struct["last_trade_won"] is True
            and session_struct["dynamics_state"] == "down"
        ):
            type = "performance_adjust_down"

        if (
            session_struct["last_trade_won"] is False
            and session_struct["dynamics_state"] == "up"
        ):
            type = "performance_adjust_down"
        ...
        if TEST_MODE:
            settings_struct["HOLDING_TIME_LIMIT"] = (
                settings_struct["TIME_DIFFERENCE"] * 60
            ) * HOLDING_INTERVAL_LIMIT
