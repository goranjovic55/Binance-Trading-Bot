...
def dynamic_settings(
    ...
    if DYNAMIC_SETTINGS:
        ...
        if (
            ...
            if session_struct["closed_trades_percent"] > (STOP_LOSS * TRADE_SLOTS):
                DYNAMIC_STOP_LOSS = (
                    ...
                    / TRADE_SLOTS
                    * Decimal(str(WIN_LOSS_PERCENT))
                    / Decimal("100")
                )
                settings_struct["STOP_LOSS"] = (
                    settings_struct["STOP_LOSS"] + DYNAMIC_STOP_LOSS
                ) / Decimal("2")
                settings_struct["TRAILING_STOP_LOSS"] = settings_struct[
                    "TRAILING_STOP_LOSS"
                ] + session_struct["profit_to_trade_ratio"] / Decimal("2")
            ...
            if session_struct["closed_trades_percent"] < (STOP_LOSS * TRADE_SLOTS):
                ...
                settings_struct["TRAILING_STOP_LOSS"] = settings_struct[
                    "TRAILING_STOP_LOSS"
                ] + (TRAILING_STOP_LOSS / DYNAMIC_MIN_MAX)
        ...
        if (
            session_struct["closed_trades_percent"]
            < settings_struct["STOP_LOSS"] * DYNAMIC_MIN_MAX
        ):
        ...
        if session_struct["last_trade_won"] == True:
            ...
            if session_struct["dynamics_state"] == "down":
                settings_struct["TIME_DIFFERENCE"] = (
                    ...
                    - (
                        settings_struct["TIME_DIFFERENCE"]
                        * settings_struct["DYNAMIC_WIN_LOSS_DOWN"]
                    )
                    / 100
                )
        ...
        if trading_struct["consecutive_loss"] > (TRADE_SLOTS / DYNAMIC_MIN_MAX):
            ...
            settings_struct["DYNAMIC_WIN_LOSS_DOWN"] = (
                settings_struct["DYNAMIC_WIN_LOSS_DOWN"]
                + (settings_struct["DYNAMIC_WIN_LOSS_DOWN"] * DYNAMIC_WIN_LOSS_DOWN)
                / 100
            )
            settings_struct["DYNAMIC_WIN_LOSS_UP"] = (
                settings_struct["DYNAMIC_WIN_LOSS_UP"]
                - (settings_struct["DYNAMIC_WIN_LOSS_UP"] * DYNAMIC_WIN_LOSS_UP) / 100
            )
            ...
            if (
                settings_struct["DYNAMIC_WIN_LOSS_DOWN"]
                < DYNAMIC_WIN_LOSS_DOWN / DYNAMIC_MIN_MAX
            ):
                settings_struct["DYNAMIC_WIN_LOSS_DOWN"] = (
                    DYNAMIC_WIN_LOSS_DOWN / DYNAMIC_MIN_MAX
                )
            if (
                settings_struct["DYNAMIC_WIN_LOSS_UP"]
                > DYNAMIC_WIN_LOSS_UP * DYNAMIC_MIN_MAX
            ):
                settings_struct["DYNAMIC_WIN_LOSS_UP"] = (
                    DYNAMIC_WIN_LOSS_UP * DYNAMIC_MIN_MAX
                )
        ...
        if type == "performance_adjust_up":
            settings_struct["STOP_LOSS"] = settings_struct["STOP_LOSS"] + (
                settings_struct["STOP_LOSS"] * DYNAMIC_WIN_LOSS_UP
            ) / Decimal("100")
            settings_struct["TAKE_PROFIT"] = settings_struct["TAKE_PROFIT"] + (
                settings_struct["TAKE_PROFIT"] * DYNAMIC_WIN_LOSS_UP
            ) / Decimal("100")
            settings_struct["TRAILING_STOP_LOSS"] = settings_struct[
                "TRAILING_STOP_LOSS"
            ] + (settings_struct["TRAILING_STOP_LOSS"] * DYNAMIC_WIN_LOSS_UP) / Decimal(
                "100"
            )
            ...
            settings_struct["DYNAMIC_CHANGE_IN_PRICE"] = settings_struct[
                "DYNAMIC_CHANGE_IN_PRICE"
            ] + (
            ...
            settings_struct["HOLDING_PRICE_THRESHOLD"] = settings_struct[
        ...
        if type == "performance_adjust_down":
            settings_struct["STOP_LOSS"] = settings_struct["STOP_LOSS"] - (
                settings_struct["STOP_LOSS"] * DYNAMIC_WIN_LOSS_DOWN
            ) / Decimal("100")
            settings_struct["TAKE_PROFIT"] = settings_struct["TAKE_PROFIT"] - (
                settings_struct["TAKE_PROFIT"] * DYNAMIC_WIN_LOSS_DOWN
            ) / Decimal("100")
            settings_struct["TRAILING_STOP_LOSS"] = settings_struct[
                "TRAILING_STOP_LOSS"
            ] - (
                settings_struct["TRAILING_STOP_LOSS"] * DYNAMIC_WIN_LOSS_DOWN
            ) / Decimal(
                "100"
            )
            settings_struct["CHANGE_IN_PRICE_MAX"] = (
        ...
        if type == "mrs_settings":
            if session_struct["prices_grabbed"] == True:
                settings_struct["CHANGE_IN_PRICE_MIN"] = session_struct[
                    "market_support"
                ] + (
                ...
                settings_struct["CHANGE_IN_PRICE_MAX"] = session_struct[
                    "market_support"
                ] - (
                    session_struct["market_support"]
                    * settings_struct["DYNAMIC_CHANGE_IN_PRICE"]
                ...
                settings_struct["TAKE_PROFIT"] = session_struct["market_resistance"] + (
