...
def trailing_buy(volatile_coins: Dict[str, Decimal]) -> Dict[str, Decimal]:
    ...
    for coin in trail_buy_coins:
        if trail_buy_historical[coin]["price"] > trail_buy_last_price[coin]["price"]:
            ...
            print(
                f"COIN: {coin} has DROPPED for {Decimal('-1.0') *(trail_buy_historical[coin]['price'] - trail_buy_last_price[coin]['price']) / trail_buy_historical[coin]['price'] * Decimal('100')}%"
            )
        ...
        if trail_buy_historical[coin]["price"] < trail_buy_last_price[coin]["price"]:
            ...
            print(
                f"COIN: {coin} has GONE UP!!!! for {Decimal('-1.0') *(trail_buy_historical[coin]['price'] - trail_buy_last_price[coin]['price']) / trail_buy_historical[coin]['price'] * Decimal('100')}%"
            )
...
def sell_coins() -> Dict:
    ...
    for coin in list(coins_bought):
        ...
        coinTakeProfit = BUY_PRICE + (
            (BUY_PRICE * coins_bought[coin]["take_profit"]) / Decimal("100")
        )
        # coinStopLoss is the price at which to 'stop losses' based on config % markdown
        coinStopLoss = BUY_PRICE + (
            (BUY_PRICE * coins_bought[coin]["stop_loss"]) / Decimal("100")
        )
        # coinHoldingTimeLimit is the time limit for holding onto a coin
        coinHoldingTimeLimit = (
            Decimal(coins_bought[coin]["timestamp"])
        ...
        if ORDER != "":
            print(
                f"{txcolors.SELL_PROFIT if priceChange >= 0. else txcolors.SELL_LOSS}TP or SL reached, selling {coins_bought[coin]['volume']} {coin}. Bought at: {BUY_PRICE} (Price now: {LAST_PRICE})  - {priceChange:.2f}% - Est: {(QUANTITY * priceChange) / Decimal('100'):.{decimals()}f} {PAIR_WITH}{txcolors.DEFAULT}"
            )
