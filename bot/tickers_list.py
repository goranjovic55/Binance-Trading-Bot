...
def tickers_list() -> None:
    ...
    if LIST_AUTOCREATE:
        ...
        if LIST_CREATE_TYPE == "tradingview":
            ...
            with open(TICKERS_LIST, "w") as f:
                for i in ta_data["data"]:
                    if (
                        i["s"][:7] == "BINANCE"
                        and i["s"][-len(PAIR_WITH) :] == PAIR_WITH
                        and i["s"][8 : -len(PAIR_WITH)] not in ignorelist
                    ):
        ...
        if LIST_CREATE_TYPE == "edgesforledges":
            url = (
                "http://edgesforledges.com/watchlists/download/binance/"
                + LIST_CREATE_TYPE_OPTION
            )
        ...
        for coin in tickers_binance:
            if any(
                ...
                if SORT_LIST_TYPE == "price_change":
                    tickers_sort[coin["symbol"]] = {
                        "priceChangePercent": Decimal(coin["priceChangePercent"])
                    }

        # sort tickers by descending order volume and price
        if SORT_LIST_TYPE == "volume":
            tickers_sort = list(
                sorted(tickers_sort.items(), key=lambda x: x[1]["volume"], reverse=True)
            )
        if SORT_LIST_TYPE == "price_change":
            tickers_sort = list(
                sorted(
                    tickers_sort.items(),
                    key=lambda x: x[1]["priceChangePercent"],
                    reverse=True,
                )
            )
