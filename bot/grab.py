    ...
    if session_struct["price_timedelta"] < current_time_minutes - round(
        ...
        if session_struct["prices_grabbed"] == True:
            ...
            for coin in historical_prices[hsp_head]:
                ...
                min_price = min(
                    historical_prices,
                    key=lambda x: Decimal("inf") if x is None else x[coin]["price"],
                )
                max_price = max(
                    historical_prices,
                    key=lambda x: Decimal("-1") if x is None else x[coin]["price"],
                )

                threshold_check = (
                    (
                        Decimal("-1.0")
                        if min_price[coin]["time"] > max_price[coin]["time"]
                        else Decimal("1.0")
                    )
