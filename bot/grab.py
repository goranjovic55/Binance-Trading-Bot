...
def get_historical_price() -> None:
    ...
    if is_fiat():
        ...
        market_historic = client.get_historical_trades(
            symbol=session_struct["exchange_symbol"]
        )
...
def get_price(add_to_historical: bool = True) -> Dict:
    ...
    while True:
        try:
            ...
            print(
                f"{txcolors.WARNING}Binance Problem Get All Tickers{txcolors.DEFAULT}"
            )
            time.sleep(0.2)
            continue
        break
    ...
    for coin in prices:
        ...
        if any(
            item + PAIR_WITH == coin["symbol"] for item in session_struct["tickers"]
        ) and all(item not in coin["symbol"] for item in EXCLUDED_PAIRS):
...
def wait_for_price(type: str) -> Tuple[Dict, float, Dict]:
    ...
    if session_struct["price_timedelta"] == 0:
    ...
    if session_struct["price_timedelta"] < current_time_minutes - round(
        ...
        if session_struct["prices_grabbed"] == True:
            ...
            # FIXME: This block should be refactored to avoid the expect..continue
            for coin in historical_prices[hsp_head]:
                ...
                min_price = min(
                    historical_prices,
                    key=lambda x: float("inf") if x is None else x[coin]["price"],
                )
                max_price = max(
                    historical_prices,
                    key=lambda x: -1 if x is None else x[coin]["price"],
                )

                threshold_check = (
                    (-1.0 if min_price[coin]["time"] > max_price[coin]["time"] else 1.0)
                    * (max_price[coin]["price"] - min_price[coin]["price"])
                ...
                if threshold_check > 0:
                    session_struct["market_resistance"] = (
                        session_struct["market_resistance"] + threshold_check
                    )
                    coins_up = coins_up + 1

                if threshold_check < 0:
                    session_struct["market_support"] = (
                        session_struct["market_support"] - threshold_check
                    )
                    coins_down = coins_down + 1

            if coins_up != 0:
                session_struct["market_resistance"] = (
                    session_struct["market_resistance"] / coins_up
                )
            if coins_down != 0:
                session_struct["market_support"] = (
                    -session_struct["market_support"] / coins_down
                )
        ...
        for coin in historical_prices[hsp_head]:
            ...
            max_price = max(
                historical_prices, key=lambda x: -1 if x is None else x[coin]["price"]
            )
            ...
            if type == "percent_mix_signal":
                ...
                if (
                    threshold_check > settings_struct["CHANGE_IN_PRICE_MIN"]
                    and threshold_check < settings_struct["CHANGE_IN_PRICE_MAX"]
                ):
                        ...
                        if excoin == coin:
                            ...
                            if coin not in volatility_cooloff:
                                volatility_cooloff[coin] = datetime.now() - timedelta(
                                    minutes=round(settings_struct["TIME_DIFFERENCE"])
                                )
                            ...
                            if datetime.now() >= volatility_cooloff[coin] + timedelta(
                                ...
                                if (
                                    session_struct["trade_slots"] + len(volatile_coins)
                                    < TRADE_SLOTS
                                    or TRADE_SLOTS == 0
                                ):
                                    volatile_coins[coin] = round(threshold_check, 3)
                                    print(
                                        f"{coin} has gained {volatile_coins[coin]}% within the last {settings_struct['TIME_DIFFERENCE']} minutes, and coin {excoin} recived a signal... calculating {QUANTITY} {PAIR_WITH} value of {coin} for purchase!"
                                    )
                                # else:
            ...
            if type == "percent_and_signal":
                ...
                if (
                    ...
                    if (
                        ...
                        volatile_coins[coin] = round(threshold_check, 3)
                        print(
                            f"{coin} has gained {volatile_coins[coin]}% within the last {settings_struct['TIME_DIFFERENCE']} minutes {QUANTITY} {PAIR_WITH} value of {coin} for purchase!"
                        )
                ...
                for excoin in externals:
                    if (
                        ...
                        and (len(coins_bought) + exnumber) < TRADE_SLOTS
                    ):
                        volatile_coins[excoin] = 1
                        exnumber += 1
                        print(
                            f"External signal received on {excoin}, calculating {QUANTITY} {PAIR_WITH} value of {excoin} for purchase!"
                        )
